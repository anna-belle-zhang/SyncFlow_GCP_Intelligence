...
class BigQueryManager:
    ...
    def create_all_tables(self) -> None:
        ...
        tables = [
            ...
            ("minietlreport", MINIETLREPORT_SCHEMA),
        ]
    ...
    def _create_priority_view(self) -> None:
        """Create or replace the SCD2 enrichment view that joins priority overrides."""
    ...
    def create_staging_table(
        ...
        """
        Create temporary staging table for MERGE operations.

        Args:
            staging_table_name: Name of staging table to create
            schema: List of SchemaField objects for table schema

        Returns:
            Full table ID (project.dataset.table)
        """
        table_id = f"{self.project_id}.{self.dataset_id}.{staging_table_name}"

        try:
            # Check if staging table exists and delete if so
            try:
                self.client.get_table(table_id)
                logger.info(f"Staging table {table_id} exists, dropping...")
                self.client.delete_table(table_id, delete_contents=True)
            except NotFound:
                logger.debug(f"Staging table {table_id} does not exist")

            # Create new staging table with auto-expiration (1 hour)
            table = bigquery.Table(table_id, schema=schema)
            table.expires = datetime.utcnow() + timedelta(hours=1)
            table.description = f"Temporary staging table for SCD2 MERGE (expires in 1 hour)"
            ...
            logger.error(f"Failed to create staging table {table_id}: {e}", exc_info=True)
            raise

    def load_records_to_staging(self, staging_table_id: str, records: List[Dict[str, Any]]) -> int:
        """
        Load SCD2 candidate records to staging table.
        ...
        """
        if not records:
            logger.warning(f"No records to load to staging table {staging_table_id}")
            return 0

        try:
            errors = self.client.insert_rows_json(staging_table_id, records)
            if errors:
                logger.error(f"Errors inserting to staging table {staging_table_id}: {errors}")
                raise RuntimeError(f"Staging table insert failed: {errors}")

            logger.info(f"✓ Loaded {len(records)} records to staging table {staging_table_id}")
            return len(records)

        except Exception as e:
            logger.error(f"Failed to load records to staging table: {e}", exc_info=True)
            raise

    def drop_staging_table(self, staging_table_id: str) -> None:
        """
        ...
        """
        try:
            self.client.delete_table(staging_table_id)
            logger.info(f"✓ Dropped staging table {staging_table_id}")
        except NotFound:
            logger.debug(f"Staging table {staging_table_id} not found (already deleted)")
        except Exception as e:
            logger.error(f"Failed to drop staging table {staging_table_id}: {e}", exc_info=True)
            raise
    ...
    def record_extraction_report(
        ...
        status: str,
        etlobjectscd2_new: int = 0,
        etlobjectscd2_updated: int = 0,
        factlog_inserted: int = 0,
        factbilling_inserted: int = 0,
        duration_sec: int = 0,
        error_message: str = None
    ) -> bool:
        """
        Record extraction metrics in minietlreport table.

        Args:
            run_id: Unique identifier for this extraction run
            extraction_phase: Phase name (INVENTORY, LOGS, BILLING)
            status: Status (SUCCESS, FAILED, WARNING)
            etlobjectscd2_new: Number of new records in etlobjectscd2
            etlobjectscd2_updated: Number of updated records in etlobjectscd2
            factlog_inserted: Number of rows inserted in factlog
            factbilling_inserted: Number of rows inserted in factbilling
        ...
        try:
            ...
            record = {
                ...
                "etlobjectscd2_new": etlobjectscd2_new if etlobjectscd2_new > 0 else None,
                "etlobjectscd2_updated": etlobjectscd2_updated if etlobjectscd2_updated > 0 else None,
                "etlobjectscd2_total": (etlobjectscd2_new + etlobjectscd2_updated) if (etlobjectscd2_new + etlobjectscd2_updated) > 0 else None,
                "factlog_inserted": factlog_inserted if factlog_inserted > 0 else None,
                "factbilling_inserted": factbilling_inserted if factbilling_inserted > 0 else None,
                "status": status,
                "error_message": error_message,
                "duration_sec": duration_sec if duration_sec > 0 else None,
            }

            table_id = f"{self.dataset_ref}.minietlreport"
            errors = self.client.insert_rows_json(table_id, [record])

            if errors:
                logger.error(f"Failed to record extraction report: {errors}")
                return False

            logger.info(f"✓ Recorded {extraction_phase} report (status={status})")
            return True

        except Exception as e:
            logger.error(f"Failed to record extraction report: {e}", exc_info=True)
            return False
